...
; BUG (fixed): SFX_RUN does not play correctly when a wild Pokémon flees from battle (see docs/bugs_and_glitches.md)
	ld de, SFX_RUN
	call WaitPlaySFX
...
.NoMoreFaintingConditions:
	...
	call HandleHealingItems ; also includes endturn abilities
	call UpdateBattleMonInParty
	call LoadTilemapToTempTilemap
	jp HandleEncore
...
; BUG (fixed): Perish Song and Spikes can leave a Pokemon with 0 HP and not faint (see docs/bugs_and_glitches.md)
.faint_loop
	call .Function
	ret c
...
.go
	...
	xor a
	ld [hl], a
; BUG (fixed): Berserk Gene's confusion lasts for 256 turns or the previous Pokémon's confusion count (see docs/bugs_and_glitches.md)
...
; BUG (fixed): A Pokémon that fainted from Pursuit will have its old status condition when revived (see docs/bugs_and_glitches.md)
	ld a, $f0
	ld [wCryTracks], a
	ld a, [wBattleMonSpecies]
	call PlayStereoCry
...
.done
; BUG (fixed): A Disabled but PP Up–enhanced move may not trigger Struggle (see docs/bugs_and_glitches.md)
	and PP_MASK
	ret nz
...
; BUG (fixed): Magikarp length limits have a unit conversion error (see docs/bugs_and_glitches.md)
	ld de, wEnemyMonDVs
	ld bc, wPlayerID
...
.CheckMagikarpArea:
; BUG (fixed): Magikarp in Lake of Rage are shorter, not longer (see docs/bugs_and_glitches.md)
	ld a, [wMapGroup]
...
; BUG (fixed): PRZ and BRN stat reductions don't apply to switched Pokémon (see docs/bugs_and_glitches.md)
	call ApplyStatusEffectOnEnemyStats
	ret
...
; Swap badges 3 (PlainBadge) and 5 (MineralBadge).
	...
	ld c, 4
.CheckBadge:
; BUG (fixed): Glacier Badge may not boost Special Defense depending on the value of Special Attack (see docs/bugs_and_glitches.md)
	ld a, b
	srl b
	push af
	call c, BoostStat
...
; BUG (fixed): Switching out or switching against a Pokémon with max HP below 4 freezes the game (see docs/bugs_and_glitches.md)
