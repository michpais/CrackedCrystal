...
PokeBallEffect:
; BUG (fixed): The Dude's catching tutorial may crash if his Poké Ball can't be used (see docs/bugs_and_glitches.md)
	ld a, [wBattleMode]
	dec a
...
; BUG (fixed): HELD_CATCH_CHANCE has no effect (see docs/bugs_and_glitches.md)
	ld d, a
	push de
	ld a, [wBattleMonItem]
	ld b, a
...
.Transformed:
	...
	; must temporarily hold caught pokemon info b/c if your pokemon levels up, the wCurPartyLevel and species get overwritten
	ld a, [wTempSpecies]
	ld l, a
	ld a, [wCurPartyLevel]
	ld h, a
...
; BUG (fixed): Love Ball boosts catch rate for the wrong gender (see docs/bugs_and_glitches.md)
	ld a, d
	pop de
	cp d
	pop bc
